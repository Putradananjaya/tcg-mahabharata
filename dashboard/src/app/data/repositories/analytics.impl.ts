...
export class AnalyticsImpl implements AnalyticsService {
  // NOTE: every series below (initialData and recalculateAnalytics's formulas) is hand-authored
  // placeholder/demo data, not output from src/simulator (Python) or any real match log. Treat
  // this whole component as UI decoration, not a data source for paper claims (see CLAIMS_LEDGER.md) —
  // e.g. `sensitivity` is generated by a literal linear formula (eqWR + (hp-95)*slope) below, so its
  // apparent linearity is a mock-data artifact, not evidence about the simulator's win-rate curve.
  private initialData: AnalyticsData = {
    sensitivity: {
      hpValues: [60, 70, 80, 90, 100, 110, 120, 130, 140],
      wrTamasika: [20, 29, 37, 46, 54, 63, 71, 80, 88],
      wrSatwika: [65, 61, 57, 52, 48, 43, 39, 35, 30],
    ...
    powerSpike: {
      ...
      satwika: [3.0, 3.0, 3.0, 2.8, 2.7, 2.7, 2.6, 2.4, 2.3, 2.1, 1.8, 1.6, 1.5, 1.3, 1.0, 0.7, 0.4, 0.2, 0.1, 0.0],
      tamasika: [3.0, 2.9, 2.8, 2.7, 2.5, 2.4, 2.2, 2.0, 1.9, 1.7, 1.5, 1.3, 1.0, 0.8, 0.5, 0.3, 0.1, 0.0, 0.0, 0.0],
  ...
  recalculateAnalytics(loss: number, p1WinRate?: number, avgTurns?: number, p1Name?: string, p2Name?: string): void {
    ...
    current.sensitivity = {
      ...
      p3Name: 'RAJASIKA',
      wrTamasika: hpValues.map(hp => Math.min(95, Math.max(10, Math.round(eqWR + (hp - 95) * slope)))),
      wrSatwika: hpValues.map(hp => Math.min(90, Math.max(10, Math.round(eqWR - (hp - 95) * 0.44)))),
